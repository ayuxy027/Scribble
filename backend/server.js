...
// Send current game state to a specific player (for reconnection/joining mid-game)
async function emitGameStateToPlayer(roomId, playerId) {
  try {
    const room = await Room.findOne({ roomId });
    if (!room || room.roundPhase !== 'drawing') return;

    // Create current word hint based on revealed characters
    let wordHint = '';
    const word = room.currentWord.toLowerCase();
    for (let i = 0; i < word.length; i++) {
      if (room.revealedChars.includes(i)) {
        wordHint += room.currentWord[i];
      } else if (word[i] === ' ') {
        wordHint += ' ';
      } else {
        wordHint += '_';
    ...
    // Send current game state to the specific player
    io.to(playerId).emit('current-game-state', {
      wordHint: wordHint,
      revealedChars: room.revealedChars,
      drawerId: room.currentDrawerId,
      round: room.currentRound + 1,
      totalRounds: room.turnOrder.length,
      isWordGuessed: room.isWordGuessed
    });

    console.log(`[Room ${roomId}] Sent current game state to player ${playerId}. Revealed chars: ${room.revealedChars.length}`);
  } catch (error) {
    console.error('Error in emitGameStateToPlayer:', error);
  }
}
...
async function startNextRound(roomId) {
  try {
    ...
    console.log(`[Room ${roomId}] Round ${room.currentRound + 1} of ${room.turnOrder.length}: ${drawerName} choosing from: [${wordOptions.join(', ')}]`);
    ...
    const autoSelectTimer = setTimeout(async () => {
      try {
        console.log(`[Room ${roomId}] Auto-selection timer triggered for ${drawerName}`);
        ...
        if (updatedRoom.roundPhase !== 'choose-word') {
          console.log(`[Room ${roomId}] Phase changed, auto-selection cancelled`);
          return;
        }
        
        if (updatedRoom.currentWord && updatedRoom.currentWord.length > 0) {
          console.log(`[Room ${roomId}] Word already chosen manually: "${updatedRoom.currentWord}"`);
          return;
        }
        ...
        console.log(`[Room ${roomId}] Auto-selected word: "${selectedWord}" for ${drawerName}`);
...
async function startDrawingPhase(roomId) {
  try {
    ...
    // Create initial word hint (underscores for letters, spaces for spaces)
    const wordLength = room.currentWord.length;
    let wordHint = '';
    for (let i = 0; i < room.currentWord.length; i++) {
      if (room.currentWord[i] === ' ') {
        wordHint += ' '; // Keep spaces as spaces
      } else {
        wordHint += '_'; // Letters as underscores
      }
    }
    ...
    // Send word hint to non-drawers
    room.users.forEach(user => {
      if (user.id !== room.currentDrawerId) {
        io.to(user.id).emit('word-hint-initial', {
          wordHint: wordHint,
          wordLength: wordLength
        });
      }
    ...
    if (global.roomTimers && global.roomTimers[roomId]) {
      if (global.roomTimers[roomId].hint1) {
        clearTimeout(global.roomTimers[roomId].hint1);
      }
      if (global.roomTimers[roomId].hint2) {
        clearTimeout(global.roomTimers[roomId].hint2);
      }
      if (global.roomTimers[roomId].drawing) {
        clearTimeout(global.roomTimers[roomId].drawing);
      }
    }
    ...
    // Set up hint reveals at exactly 10 and 20 seconds
    global.roomTimers[roomId].hint1 = setTimeout(async () => {
      await revealRandomCharacter(roomId, 1); // First hint after 10 seconds
    }, 10000);

    global.roomTimers[roomId].hint2 = setTimeout(async () => {
      await revealRandomCharacter(roomId, 2); // Second hint after 20 seconds
    }, 20000);

    // Drawing timer (30 seconds)
    global.roomTimers[roomId].drawing = setTimeout(async () => {
      try {
        const updatedRoom = await Room.findOne({ roomId });
...
async function revealRandomCharacter(roomId, hintNumber) {
  try {
    const room = await Room.findOne({ roomId });
    if (!room || room.roundPhase !== 'drawing' || room.isWordGuessed) {
      console.log(`[Room ${roomId}] Hint ${hintNumber} skipped - invalid state`);
      return;
    }
    ...
    if (availablePositions.length === 0) {
      console.log(`[Room ${roomId}] Hint ${hintNumber} skipped - no more characters to reveal`);
      return;
    }

    // Pick a random position to reveal (ensure exactly 1 character)
    const randomIndex = Math.floor(Math.random() * availablePositions.length);
    const positionToReveal = availablePositions[randomIndex];
    
    // Add the position to revealed characters array
    room.revealedChars.push(positionToReveal);
    
    // Save to database immediately to ensure synchronization
    await room.save();
    ...
    for (let i = 0; i < word.length; i++) {
      if (room.revealedChars.includes(i)) {
        wordHint += room.currentWord[i]; // Show the actual character (preserve case)
      } else if (word[i] === ' ') {
    ...
    const hintData = {
      ...
      totalRevealed: room.revealedChars.length
    };
    ...
    console.log(`[Room ${roomId}] Hint ${hintNumber}: Revealed character '${room.currentWord[positionToReveal]}' at position ${positionToReveal}. Total revealed: ${room.revealedChars.length}. Hint: "${wordHint}"`);

  } catch (error) {
    console.error(`Error in revealRandomCharacter for room ${roomId}, hint ${hintNumber}:`, error);
  }
}
...
async function showRoundResults(roomId) {
  try {
    ...
    if (global.roomTimers && global.roomTimers[roomId]) {
      if (global.roomTimers[roomId].hint1) {
        clearTimeout(global.roomTimers[roomId].hint1);
        global.roomTimers[roomId].hint1 = null;
      }
      if (global.roomTimers[roomId].hint2) {
        clearTimeout(global.roomTimers[roomId].hint2);
        global.roomTimers[roomId].hint2 = null;
      }
      if (global.roomTimers[roomId].drawing) {
        clearTimeout(global.roomTimers[roomId].drawing);
        global.roomTimers[roomId].drawing = null;
      }
    }
...
io.on('connection', (socket) => {
    ...
    try {
      ...
      if (userIndex !== -1) {
        ...
        const formattedHistory = chatHistory.map(msg => ({
          ...
          message: msg.data.isGuess ? `[Guessed: ${msg.data.word}]` : msg.data.message,
        ...
        if (room.gameState === 'playing') {
          ...
          if (room.roundPhase === 'choose-word') {
            ...
            // Send current game state with revealed characters (for all players except drawer)
            if (room.currentDrawerId !== socket.id) {
              await emitGameStateToPlayer(roomId, socket.id);
            }
  ...
  socket.on('create-room', async (username) => {
    try {
      ...
      // Emit initial ready status (host is always ready)
      const readyStatus = { [socket.id]: true };
      io.in(roomId).emit('ready-status-update', readyStatus);
    ...
    try {
      ...
      room.users.forEach(user => {
        if (user.id === room.hostId) {
          readyStatus[user.id] = true; // Host is always ready
        } else {
          readyStatus[user.id] = room.readyPlayers && room.readyPlayers.includes(user.id);
        }
      });
      io.in(roomId).emit('ready-status-update', readyStatus);
  ...
  socket.on('start-game', async (roomId) => {
    try {
      ...
      const nonHostPlayers = room.users.filter(u => u.id !== room.hostId);
      const readyPlayers = room.readyPlayers || [];
      const allPlayersReady = nonHostPlayers.every(player => readyPlayers.includes(player.id));
      
      if (!allPlayersReady) {
        const readyCount = readyPlayers.length;
        const totalNonHost = nonHostPlayers.length;
        return socket.emit('error', `Not all players are ready. ${readyCount}/${totalNonHost} players ready.`);
      }
      ...
      room.currentRound = 0;
      room.availableWords = [...words];
      room.readyPlayers = []; // Reset ready status for game
  ...
  // Handle ready status toggle for lobby
  socket.on('toggle-ready', async (roomId) => {
    try {
      const room = await Room.findOne({ roomId });
      if (!room) return socket.emit('error', 'Room not found');
      if (room.gameState !== 'lobby') return socket.emit('error', 'Can only toggle ready in lobby');
      
      const user = room.users.find(u => u.id === socket.id);
      if (!user) return socket.emit('error', 'User not found in room');
      
      // Host is always considered ready, don't allow toggle
      if (socket.id === room.hostId) {
        return socket.emit('error', 'Host is always ready');
      }
      
      // Initialize readyPlayers array if it doesn't exist
      if (!room.readyPlayers) room.readyPlayers = [];
      
      // Toggle ready status
      const isCurrentlyReady = room.readyPlayers.includes(socket.id);
      if (isCurrentlyReady) {
      ...
      // Create ready status object for each user
      const readyStatus = {};
      room.users.forEach(user => {
        if (user.id === room.hostId) {
          readyStatus[user.id] = true; // Host is always ready
        } else {
          readyStatus[user.id] = room.readyPlayers.includes(user.id);
        }
      });
      ...
      console.log(`[Room ${roomId}] ${user.username} ${isCurrentlyReady ? 'unready' : 'ready'} (${room.readyPlayers.length}/${room.users.length - 1})`);
    } catch (error) {
  ...
  socket.on('word-chosen', async (roomId, chosenWord) => {
    try {
      const room = await Room.findOne({ roomId });
      if (!room || room.currentDrawerId !== socket.id || room.roundPhase !== 'choose-word') {
        console.log(`[Room ${roomId}] Invalid word choice attempt by ${socket.id}`);
        return;
      }
      
      // Verify the chosen word is from the available options
      if (!room.tempWordOptions || !room.tempWordOptions.includes(chosenWord)) {
        console.log(`[Room ${roomId}] Invalid word selection: "${chosenWord}" not in options`);
        return socket.emit('error', 'Invalid word selection');
      }
      
      // Clear auto-selection timer
      if (global.roomTimers && global.roomTimers[roomId] && global.roomTimers[roomId].autoSelect) {
        clearTimeout(global.roomTimers[roomId].autoSelect);
        delete global.roomTimers[roomId].autoSelect;
        console.log(`[Room ${roomId}] Auto-selection timer cleared - word chosen manually`);
      }
      
      room.currentWord = chosenWord;
      await room.save();
      
      const drawerName = room.users.find(u => u.id === socket.id)?.username || 'Unknown';
      console.log(`[Room ${roomId}] ${drawerName} manually chose word: "${chosenWord}"`);
  ...
  socket.on('send-message', async ({ roomId, message }) => {
    try {
      ...
      if (isCorrect) {
        ...
        if (drawer && !room.roundScores.find(score => score.userId === drawer.id)) {
          room.roundScores.push({
            userId: drawer.id,
            points: 50,
        ...
        socket.to(roomId).emit('system-message', `${user.username} guessed the word! (+${points} pts)`);
